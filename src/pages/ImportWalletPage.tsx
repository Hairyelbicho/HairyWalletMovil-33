...
const ImportWalletPage = () => {
  ...
  const handleImport = async (e: FormEvent) => {
    ...
    try {
      ...
      // Después de importar correctamente, vamos al dashboard principal
      navigate("/dashboard");
    } catch (err) {
      const message =
        err instanceof Error
          ? err.message
          : "Error al importar la wallet. Revisa los datos introducidos.";
      setError(message);
    }
  };
        ...
        <div className="bg-white/10 backdrop-blur-md rounded-2xl p-8 border border-white/20 relative">
          ...
          <div className="text-center mb-8">
            <div className="flex justify-center mb-4">
              <div className="w-16 h-16 bg-gradient-to-br from-blue-400 to-indigo-500 rounded-full flex items-center justify-center">
                <i className="ri-download-cloud-fill text-3xl text-white" />
              </div>
            </div>
            <h1 className="text-3xl font-bold text-white mb-2">
              Importar Billetera
            </h1>
            <p className="text-purple-200">Recupera tu wallet existente</p>
          </div>
          ...
          {error && (
            <div className="mb-6 bg-red-500/20 border border-red-400/30 rounded-xl p-4">
              <div className="flex items-center space-x-3">
                <i className="ri-error-warning-fill text-2xl text-red-400" />
          ...
          <form onSubmit={handleImport} className="space-y-6">
            {importMethod === "mnemonic" ? (
              <div>
                ...
                <p className="text-xs text-purple-300 mt-2">
                  Ejemplo: word1 word2 word3 word4 word5 word6 word7 word8 word9
            ...
            >
              {isLoading ? (
                ...
                <>
                  <i className="ri-download-cloud-fill text-xl" />
          ...
          <div className="mt-6 bg-yellow-500/10 border border-yellow-400/30 rounded-xl p-4">
            <div className="flex items-start space-x-3">
              <i className="ri-shield-check-line text-xl text-yellow-400 flex-shrink-0" />
              <div className="text-xs text-yellow-200">
                <p className="font-semibold mb-2">🔒 Seguridad:</p>
                <ul className="space-y-1 list-disc list-inside">
                  <li>
                    Tu frase semilla y clave privada nunca se envían a ningún
                    servidor
                  </li>
                  <li>
                    Se almacenan de forma segura solo en tu dispositivo
                  </li>
                  <li>
                    Asegúrate de estar en un entorno seguro antes de ingresar
                    tus claves
                  </li>
                </ul>
              </div>
